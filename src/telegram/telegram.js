...
export async function sendWithButtons(message, chatId, buttons, botToken) {
  return sendTelegram(message, chatId, botToken, { keyboard: { inline_keyboard: buttons } });
}
...
export async function sendLongTelegram(message, chatId, botToken, options = {}) {
  ...
  if (message.length <= MAX_LENGTH) return sendTelegram(message, chatId, botToken, options);
