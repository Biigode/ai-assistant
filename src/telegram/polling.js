...
// Polling do Telegram — processa mensagens de texto E cliques em botões
...
import { extractInterests } from '../ai/llm.js';
...
async function processUpdate(update, token) {
  ...
  if (text.startsWith('/')) {
    await handleTextCommand(chatId, text.split(' ')[0].toLowerCase(), name, token);
    return;
  }
...
async function handleTextCommand(chatId, cmd, name, token) {
  const { findUserByChatId } = await import('../../models/UserPreferences.js');
  const user = await findUserByChatId(chatId);
  ...
  switch (cmd) {
    ...
    case '/noticias':  await processNoticias(chatId, user, token); break;
    case '/digest':    await processDigest(chatId, name, token); break;
    case '/perfil':
    case '/profile':   await processProfile(chatId, user, token); break;
    case '/adicionar':
    case '/add':
      ...
      break;
    case '/remover':
    case '/remove':    await processRemover(chatId, user, token); break;
    case '/horario':
    case '/schedule': {
    ...
    case '/help':
    case '/ajuda':     await processAjuda(chatId, token); break;
    default: {
      // Comando desconhecido — trata como mensagem normal
      await handleChatMessage(chatId, cmd, name);
    }
  }
}
...
async function processCallback(callbackQuery, token) {
  ...
  // Comandos diretos
  if (data === 'cmd:noticias')  { await answerCallback(queryId, '📰 Buscando...', token); await processNoticias(chatId, user, token); return; }
  if (data === 'cmd:digest')    { await answerCallback(queryId, '📦 Gerando...', token);  await processDigest(chatId, name, token); return; }
  if (data === 'cmd:perfil')    { await answerCallback(queryId, '', token); await processProfile(chatId, user, token); return; }
  if (data === 'cmd:ajuda')     { await answerCallback(queryId, '', token); await processAjuda(chatId, token); return; }
  ...
  if (data === 'cmd:remover') { await answerCallback(queryId, '', token); await processRemover(chatId, user, token); return; }
  ...
  if (data.startsWith('estilo:')) {
    ...
    await salvarEstilo(chatId, user, data.replace('estilo:', ''), token);
    return;
  }
...
async function handleInteractiveStep(chatId, text, name, user, token, currentStep) {
  ...
  switch (currentStep.step) {
    ...
    case 'horario_custom': {
      ...
      await salvarHorario(chatId, user, `0 ${parseInt(match[1])} * * *`, token);
      return null;
    }
    default:
      await handleChatMessage(chatId, text, name);
...
async function processNoticias(chatId, user, token) {
  ...
  if (!interests.length) {
    ...
    await sendWithButtons('⚠️ Você não tem interesses configurados.\nUse ⚙️ Configurações para adicionar!', chatId, menu.buttons, token);
    return;
  }
...
async function processResearch(chatId, tema, token) {
  await sendLongTelegram(`🔬 Iniciando investigação profunda sobre "${tema}"...\nVou buscar múltiplas fontes e gerar um briefing completo para seu vídeo!`, chatId, token);
  try {
...
async function processRemover(chatId, user, token) {
  ...
  const buttons = interests.map(i => ([{ text: `❌ ${i.topic}`, callback_data: `remove_interest:${i.topic}` }]));
...
async function salvarHorario(chatId, user, cron, token) {
  const { createOrUpdateUser } = await import('../../models/UserPreferences.js');
  await createOrUpdateUser({ ...user, telegramChatId: chatId, digestSettings: { ...(user?.digestSettings || {}), cronSchedule: cron } });
...
async function salvarEstilo(chatId, user, estilo, token) {
  const { createOrUpdateUser } = await import('../../models/UserPreferences.js');
  await createOrUpdateUser({ ...user, telegramChatId: chatId, digestSettings: { ...(user?.digestSettings || {}), summaryStyle: estilo } });
