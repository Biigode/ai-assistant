...
async function searchDDGInstant(query) {
  ...
  if (data.AbstractText && data.AbstractURL) {
    results.push({ title: data.Heading || query, link: data.AbstractURL, snippet: truncate(data.AbstractText, MAX_SNIPPET), source: data.AbstractSource || toHostname(data.AbstractURL), publishedAt: null });
  }
  for (const topic of data.RelatedTopics || []) {
    if (!topic.FirstURL || !topic.Text) continue;
    results.push({ title: truncate(topic.Text, 100), link: topic.FirstURL, snippet: truncate(topic.Text, MAX_SNIPPET), source: toHostname(topic.FirstURL), publishedAt: null });
...
export async function searchTopic(topic, num = 5) {
  ...
  for (const s of strategies) {
    try {
      ...
      if (results.length > 0) { logSearch(topic, results, s.name); return results; }
    } catch (err) {
