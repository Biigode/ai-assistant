...
export async function addMessage(chatId, role, content) {
  ...
  await col.insertOne({ chatId: chatId.toString(), role, content, createdAt: new Date() });

  const count = await col.countDocuments({ chatId: chatId.toString() });
  if (count > MAX_MESSAGES) {
    const oldest = await col.find({ chatId: chatId.toString() }).sort({ createdAt: 1 }).limit(count - MAX_MESSAGES).toArray();
    if (oldest.length > 0) await col.deleteMany({ _id: { $in: oldest.map(d => d._id) } });
  }
}
...
export async function buildContextString(chatId) {
  ...
  return history.map(m => `${m.role === 'user' ? 'Usuário' : 'Assistente'}: ${m.content}`).join('\n');
}
