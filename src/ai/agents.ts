...
import { getCurrentDateContext, searchTopic } from "../features/core/search.ts";
import type {
  Analysis,
  AnalystResult,
  Briefing,
...
import { getReasoningModel } from "./model-router.ts";

export async function researcherAgent(topic: string): Promise<SearchResult[]> {
  ...
  try {
    const res = await ollama.generate({
      ...
      options: {
        ...
        ...(mode === "gpt-oss" ? { reasoning_effort: "high" } : {}),
      },
    });
    const match = res.response.trim().match(/\{[\s\S]*\}/);
    if (!match) throw new Error("JSON não encontrado");
    const analysis: Analysis = JSON.parse(match[0]);
    console.log(`   ✅ Análise concluída via ${mode}`);
    return { analysis, sources, mode };
  } catch (err) {
    console.error(`   ❌ Analyst falhou:`, (err as Error).message);
...
export async function writerAgent(
  ...
  try {
    ...
    console.error(`   ❌ Writer falhou:`, (err as Error).message);
...
export async function runResearchPipeline(topic: string): Promise<Briefing> {
  const { label } = getCurrentDateContext();
  ...
  try {
    ...
    console.warn(
      "⚠️  Não foi possível salvar no MongoDB:",
